...
> 3. `skill_seguranca-webapp.md` - diretrizes de segurança
---
...
| 2026-03-29 | Mobile: campos horaEntrada, horaSaida e totalHoras agora exibidos na mesma linha no MobileModal — CSS grid 3 colunas com grid-column:1/-1 para número/cargo/nome e auto-placement para os campos de hora. 148 testes passando. |
| 2026-03-29 | Layout padrão mobile: MobileLancamentosPage alinhada ao design system do desktop — header #212B36 + borda #2E3B4A; título "Controle de " #F8FAFC + "Terceiros" #F37E38 (brand); badge Mobile em chip laranja; botões PT/EN e Desktop com paleta do header desktop; fundo da página #FAF9FB + font-family DM Sans; MobileModal header #212B36 + corpo #FAF9FB; FAB #F37E38 (brand orange); inputs dos filtros border 1.5px #E8E8EA + bg #FAFAFA; empty state emoji substituído por SVG inline. 148 testes passando. |
...
| 2026-03-29 | Fix auth: signOut da sessão aal1 ao clicar "← Voltar ao login" no MFA — elimina race condition que forçava retorno ao step MFA em menos de 1s; handler handleBackToLogin reseta todo estado MFA (factorId, challengeId, step, mfaCode). Fix route guard: hasMfaPending() mantém LoginPage montado para sessões aal1 com MFA pendente; rotas internas (/, /admin, /mobile) bloqueadas para sessões aal1. 127 testes passando. |
| 2026-03-29 | Melhorias de qualidade: console.log → console.debug em useAuthStatus.ts, useStorage.ts e supabase.ts (12 ocorrências — silenciados em produção, visíveis em dev); removida linha "noImplicitAny": false do tsconfig.app.json (strict: true agora funciona completamente — zero erros TypeScript confirmados); supabase/schema/schema.sql reescrito como snapshot consolidado pós-19-migrations (10 tabelas, 8 funções, 3 triggers, todos os índices, RLS AAL2 final). 148 testes passando. |
| 2026-03-29 | Correções de segurança Supabase Advisor: migration fix_security_advisors_2026.sql — (1) lint 0011: criar_fechamento e excluir_fechamento reescritas com SET search_path = public, pg_catalog e tabelas fully-qualified (public.fechamentos, public.fechamento_itens); (2) lint 0014: pg_trgm movida para schema extensions (DROP CASCADE + CREATE WITH SCHEMA extensions + recriação do índice GIN); (3) lint 0012 – limpeza defensiva de policies anon residuais (DROP IF EXISTS); fix principal do lint 0012: desabilitar Anonymous Sign-Ins no Dashboard (Authentication → Configuration) + habilitar Leaked Password Protection (Authentication → Security). |
