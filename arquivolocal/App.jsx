...
const calcHoras  = (e, s) => { if (!e || !s) return ""; const [eh,em]=e.split(":").map(Number),[sh,sm]=s.split(":").map(Number); let m=(sh*60+sm)-(eh*60+em); if(m<0)m+=1440; return `${String(Math.floor(m/60)).padStart(2,"0")}:${String(m%60).padStart(2,"0")}`; };
...
// ─── STORAGE (localStorage para VS Code) ────────────────────────
const useStorage = (key, seed) => {
...
const Chip = ({ label, color="#1A56DB", bg, size="sm" }) => (
...
const Badge = ({ n, color="#E02424" }) => n > 0 ? (
...
const Btn = ({ children, onClick, variant="primary", small, icon, disabled, full, style:s }) => {
  const V = { primary:{bg:"#1A56DB",c:"#fff"}, ghost:{bg:"#F1F5F9",c:"#334155"}, danger:{bg:"#FDE8E8",c:"#E02424"}, success:{bg:"#E6F9F4",c:"#0E9F6E"}, warning:{bg:"#FEF3C7",c:"#B45309"}, outline:{bg:"transparent",c:"#1A56DB",border:"1.5px solid #1A56DB"} };
...
const Modal = ({ title, subtitle, onClose, children, wide, xl }) => (
...
// ─── FORM DE LANÇAMENTO ─────────────────────────────────────────
const FormLancamento = ({ inicial, onSave, onCancel }) => {
  ...
  const G = ({ children, cols=2 }) => <div style={{ display:"grid", gridTemplateColumns:`repeat(${cols},1fr)`, gap:14 }}>{children}</div>;
  const S = ({ children, span=1 }) => <div style={{ gridColumn:`span ${span}` }}>{children}</div>;
...
// ─── TELA: LANÇAMENTOS ──────────────────────────────────────────
const Lancamentos = ({ registros, setRegistros }) => {
  const [filtros, setFiltros] = useState({ data: hoje(), turno:"", fornecedor:"", unidade:"", setor:"", busca:"" });
  const [modal, setModal]     = useState(null); // null | "new" | reg
  ...
  const excluir = (id) => { setRegistros(registros.filter(r=>r.id!==id)); setConfirm(null); };
  ...
  const FORN_CORES = { "LIDER MASTER":"#D97706", "TRANSLOG":"#1A56DB", "SERVILOG":"#0E9F6E", "LOGFLEX":"#6C63FF", "OUTRO":"#64748B" };
    ...
    <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
      ...
      {modal && (
        <Modal title={modal==="new"?"Novo Lançamento":"Editar Lançamento"} subtitle="Controle de Terceiros" onClose={()=>setModal(null)} xl>
          <FormLancamento inicial={modal==="new"?null:modal} onSave={salvar} onCancel={()=>setModal(null)} />
        </Modal>
      )}
...
const Dashboard = ({ registros }) => {
  ...
  // Por dia (últimos 15 dias do mês)
  ...
  const KPI = ({ label, value, sub, color="#1A56DB", icon }) => (
            ...
            <div key={forn} style={{ background:"#fff", border:`1px solid ${cor}33`, borderRadius:12, overflow:"hidden", boxShadow:"0 1px 4px rgba(15,28,46,.06)" }}>
              ...
              <div style={{ padding:"14px 18px" }}>
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:10, marginBottom:14 }}>
                  {[["Hoje",hj,cor],["Este Mês",mes,"#334155"],["Total Horas",horas,"#0E9F6E"]].map(([l,v,c])=>(
...
export default function App() {
  ...
  const wrap = (fn) => (val) => { fn(val); setSaved(true); setTimeout(()=>setSaved(false), 2000); };
