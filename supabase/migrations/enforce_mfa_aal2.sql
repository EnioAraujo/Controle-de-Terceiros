...
CREATE OR REPLACE FUNCTION auth.is_aal2()
RETURNS BOOLEAN
LANGUAGE SQL
STABLE
SECURITY DEFINER
...
CREATE POLICY "aal2_insert_registros"
  ...
  WITH CHECK (auth.uid() IS NOT NULL AND auth.is_aal2());

CREATE POLICY "aal2_update_registros"
  ON public.registros FOR UPDATE TO authenticated
  USING     (auth.uid() IS NOT NULL AND auth.is_aal2())
  WITH CHECK (auth.uid() IS NOT NULL AND auth.is_aal2());
...
CREATE POLICY "aal2_profiles_update"
  ...
  USING     (auth.uid() = id AND auth.is_aal2())
  WITH CHECK (auth.uid() = id AND auth.is_aal2());
